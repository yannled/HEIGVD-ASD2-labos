...
//
//  EXACT SAME FILE AS TernarySearchTrieAVL, with wilcard function additionnal

#ifndef SearchTrees_TST_TernarySearchTrie_AVL_WILDCARD_h
#define SearchTrees_TST_TernarySearchTrie_AVL_WILDCARD_h

#include <algorithm>
#include <list>

template < typename KeyType, typename ValueType >
class TernarySearchTrieAVLWildCard {
private:
    ...
    public:
        ...
        TernarySearchTrieAVLWildCard<KeyType, ValueType> *nextRow; // MODIFIED: added
...
public:
    ...
    TernarySearchTrieAVLWildCard() : root(nullptr) {
    }
    ...
    Node* put(Node* x, const KeyType* key, ValueType value) {
        ...
        if (*key < x->key)
            ...
            if (key == nullptr || *(key) == '\0') {
                ...
                if (x->nextRow == nullptr) {
                    x->nextRow = new TernarySearchTrieAVLWildCard();
                }
...
public:
    ...
    std::list<std::string> wildCardMatch(const std::string& pattern) {
        //modifies the string to a char* ... -> a pointer on keys!
        return wildCardMatchS(pattern.c_str());
    }
private:
    // MODIFIED: ADDED

    std::list<std::string> wildCardMatchS(const KeyType* key) {
        // starting by the root
        return find(key, root);
    }
    // MODIFIED: ADDED

    std::list<std::string> find(const KeyType* key, Node* x) {
        ...
        bool wildcard = (*key == '.');
        // wilcard match: we must go everywhere, right, left and middle
        // otherwise, usual condition apply.
        if (wildcard || *key < x->key) {
            // we add all the results found to the left at the end of the list
            std::list < std::string > add = find(key, x->left);
            mylist.splice(mylist.end(), add);
        }
        if (wildcard || *key > x->key) {
            // we add all the results found to the right at the end of the list
            std::list < std::string > add = find(key, x->right);
            mylist.splice(mylist.end(), add);
        }
        if (wildcard || x->key == *key) {

            ++key; // ptr incremented (next key) ... current key is x->key!
            if (key == nullptr || *(key) == '\0') {
                // reached the end of keys: 
                // check if value is nullptr (false) or contains date.
                // 
                if (x->value != nullptr) {
                    // data: just add the key (its the end of a word).
                    std::string a = "";
                    a += (char) x->key;
                    mylist.push_back(a);
                } // false: nothing to add.

                // keys remaining but no next row: false. Nothing to add.
            } else if (x->nextRow != nullptr) {
                // keys remaining: searching in next row
                // we add all the results found to in the following row at the end of the list
                // results will be generated when coming back from the recursion
                // with the current key attached to all the results found in the next row,
                std::list < std::string> ret = x->nextRow->wildCardMatch(key);
...
#endif // de SearchTrees_TST_TernarySearchTrie_AVL_WILDCARD_h
